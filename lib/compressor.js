...
Compressor.prototype.compress = function() {
    ...
    if(!_.isEmpty(splittedList['uncached'])) {
        ...
        // start compressions
        if(this.hasFlag('preprocess')) {
            codeList = this.preprocess(codeList);
        }
...
/**
 * Use different precompressor for extensions
 */
Compressor.prototype.preprocess = function(list) {

    var processedList = {};

    helper.mapList(list, function(type, code, filepath) {

        switch(type) {
            case 'sass':
            case 'scss':
                processedList['css'] = processedList['css'] || {};
                processedList['css'][filepath] = sass.renderSync(code, {
                    includePaths: [require('path').dirname(this.cache.assets + filepath)]
                });
                break;

            default:
                processedList[type] = processedList[type] || {};
                processedList[type][filepath] = code;
        }
